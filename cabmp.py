...
"""Create a SVG of a CA"""
def caSVG(length, width, filename, rR, gR, bR, seed):
    def drawCell( c, x, y, w ):
        c.rectangle (x/width, y/length, w/width, w/length)

    fo = file( f+'.svg', 'w' )
    surface = cairo.SVGSurface (fo, width, length)
    ctx = cairo.Context (surface)
    ctx.scale (width, length)
    ...
    if seed == 'm':
        ...
        p[middle, 0] = (0, 0, 0)
        drawCell(ctx, middle+0.0, 0.0, 1.0)
    else:
        random.seed(seed)
    ...
    for row in range(1, length):
        for i in range(0, width):
            ...
            if p[i, row][0] == 0:
                print "k"
                drawCell(ctx, i+0.0, row+0.0, 1.0)
...
def caCatalog( iGen, iWidth, filename ):
    gutter_x = 6
    gutter_y = 2

    img = Image.new('RGB', ((iWidth+gutter_x)*16+gutter_x, (iGen+gutter_y)*16+gutter_y), "white")
    print img.size
    ...
    def applyRule(r, p):
        ...
        for parent in range(0,3):
            if p[parent][0] == 255:
                intp += 2 ** (2-parent)
        return (0,0,0) if r[intp] == 1 else (255,255,255)

    def drawSubrectangle( xo, yo, rule ):
        # TODO - Enter initial row
        x_off = xo*( iWidth+gutter_x )+gutter_x
        y_off = yo*( iGen+gutter_y )+gutter_y
        middle = iWidth/2
        p[middle+x_off, y_off] = (0,0,0)
        for gen in range(1, iGen):
            for i in range(0, iWidth ):
                parents = getParents(p,
                            i+x_off, gen+y_off,
                            x_off, x_off+iWidth)
                p[i+x_off, gen+y_off] = applyRule( rule, parents )

    for r in range(0, 256):
        x, y = r/16, r%16
        rule = bitfield(r)
        drawSubrectangle(x, y, rule)
